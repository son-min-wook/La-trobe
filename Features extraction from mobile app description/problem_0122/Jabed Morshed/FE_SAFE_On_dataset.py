...
if __name__ == '__main__':
    ...
    ############
    example_description = """Tired of searching your frequently used numbers on your contact list ?Well, here is your solution, Gesture Call !Call or text to any contact you want drawing a simple gesture on the screen of your device. For instance, draw an M to call Mark or maybe a heart to call your boyfriend/girlfriend.Just like this you&#39;ll be calling anyone you want !The steps are simple:1. Select a contact2. Create a gesture for him/herNow, you can call this contact anytime you want just by drawing this gesture on the main screen !YOU CAN CREATE AS MANY GESTURES AS YOU WANT !!Still not confident with your drawing skills?No problem, until you decide to change it, Gesture Call will ask you for confirmation before performing any call.You can also use a notification bar icon to access Gesture Call anytime while using your device.Gesture Call can be configured with a different themes!Features:\t? Call or text with a gesture\t? Simple, easy and fast interface\t? Unilimited gestures\t? Notification bar icon (optional)\t? Themes\t? Custom confirmation delay (if any)Gesture Call is available in:\t?English\t?Spanish\t?French\t?Catalan\t?Japanese\t?Thai\t?Italian\t?German\t?Chinese\t?Malay\t?Korean\t?RussianTags: call, calls, quick, quick call, easy, gesture, fast, cool, widget, gesture search,call one click.ll&quot; dialog.<br>? The Gestures now change the color according to the Theme.<br>? Tested the app in Android 4.0"
    """
